...
Discuss the extend of which of the risk we discusses before (Mod 3?) can be amenable to quantitative analysis
...
Use of scenario analysis and stress testing in the risk measurement process (pros and cons of each)
...
* $Y_{n,t}$: Value of the n^th^ variable at time $t$
* $X_{i,t}$: Value of the i^th^ factor at time $i$
* $\beta_{i,n}$: Weight give to the i^th^ factor in respect of the n^th variable
* $\epsilon_{n,t}$: Error term (difference between modeled and actual)

Factors are found using regression analysis
...
Different ways to quantify risk for different risk types

***Enterprise Risk: Dynamic Financila Analysis***

* Modeling risk that the enterprise as a whole exposed and the relationship between these risks
* Output from the model is typically cashflow information used to produce projected b/s and p&l accounts
* Method of assessing company's capital is typically a form of dynamic financial analysis (mod 30)

***Enterprise Risk: Financial Condition Reports***

* Report of the current solvency position of a company and is possible future development
* Needs to consider the risk it is exposed
...
***Underwriting Risk: Underwriting Modeling or Reviews***

Insurance ad underwriting risk are quantitative by nature so quantitative models are used to assess
...
e.g. Modeling the u/w-ing risk of a life insurance company issuing a policy to a smoker for a premium relevant to a non-smoker
...
Interest rate models have considered short-term, long-term interest rates and full yield curve
...
Typically use time series (Mod 17) and risk measures like VaR and TVaR
...
Risk is usually assess using both quantitative and non-quantitative criteria (more on mod 23)

***Liquidity Risk: Asset Liability Modeling***

Can be modeled quantitatively but haven't been done much in the past
...
* For liquidity risk, we are interested in the level of cash held in each period to ensure that short-term liabilities can continue to be met with a desired level of confidence

***Operational Risk: Internal and External Loss Data, Scenario Analysis, Simulations***
...
Increasingly orgs are collecting historical data on op-risk loss to further along quantitative analysis (see mod 24)

Can use external loss data

* Provide information of situations the company has not faced (supplement)
* Benchmark internal data

Regardless, quantitative analysis will never be substitute for the use of worst case scenarios (only one is required to take down the company)
...
* Use previous experiences and incorporate learning points from past events into ERM strategy with an aim to become better able to react appropriately to surprising events
* Develop and emerging risk register of potential future issues

Need to **focus on the tails** of probability distributions

* Avoid simplifying assumptions (e.g. normal distribution)
* Extreme value theory (mod 20)

***Data Limitation***

Data might be limited in volume and/or heterogeneous

Can supplement with alternative data source but can also introduce new issues (e.g. lower reliability)

***Interdependence of Risks***

Imperative to consider the interrelationships between risks (key to ERM)

Need to model many different risk factors and their dependencies within the same model (multivariate distribution)

* More in Sec 2 or Mod 16 for multivariate distributions
...
Can use risk ranges, risk buckets to analyze (due to the lack of granularity) and display the results in a risk map (mod 13)

## Correlation

Correlation: Measure of how different variables relate or associate to each other
...
For discrete sample size $T$

**Sameple correlation coefficient**:
...
* Can't be used for some heavy tailed distributions

$rho = 0$ does not imply that there is no relationship between variables (only there is **no linear** relationship)

It is not guaranteed that we can put together a joint distribution for any given r.v. $X$, $Y$, and $\rho$

* Some $\rho$ might be unattainable i.e. incompatible with the marginal distributions
...
The 2 main ones are Spearman's $\rho$ and Kendall's $\tau$

***Advantages*** over linear correlation

Rank correlation of a bivariate distribution is **independent of the marginal distribution** $\therefore$ has more attractive properties

#### Spearman's Rho

For r.v. $X$ and $Y$ w/ marginal distribution functions $F_X$ and $F_Y$

Spearman's $rho$: $_{s}\rho(X,Y) = \rho \left( F_X(X), F_Y(Y)\right)$

* $_{s}\rho$ is the linear correlation of the distribution functions of the two r.v.

For 2 r.v. $X$ and $Y$ and sample size $T$:

**Spearman's sample $\rho$**
...
2. Calculate tail dependence (mod 18)
    * Definition of tail is subjective (cut off point)
...
* For each set of assumptions, the output from the model is fully determined (no random element)
    * Contrast the unpredictable output of a truly stochastic model (not based on pseudo randomness)

Prudence is allowed for through the particular choice of assumptions e.g. by adding margins to best estimate assumptions
...
2. Provide insight into the dependence of the output on subjective assumptions
    ...
    * Varying the model's inputs can show how sensitive the model is to changes in those inputs $\therefore$ focus attention on the most important assumptions and make clear the model's reliance upon judgement
...
1. Decide top down on the scenarios to be modeled
    ...
    * Ask participants for the worst plausible event they can imagine e.g. collapse of a major financial institution, EQ, oil shortage etc
...
* Only looks at extreme situations, so needs to be coupled with other techniques (e.g. simulations) in order to understand the full range of outcomes
...
Key benefits: provides a probability distribution for the model outputs
...
* May **underestimate uncertainty** (past might not capture what potentially can happen) and so in practice should generally be used with other methods (e.g. stress test) so as to consider a greater range of outcomes

### Forward-looking Approaches

#### Monte Carlo Simulation

Simulation based on random numbers to generate input values and the model is run using these values (e.g. using $U(0,1)$ and look up $F^{-1}(u)$)
...
* Large sets of simulation may be time consuming to perform (but you can use methods such as latin hypercube to reduce the calculation burden)
...
Appear no to follow a pattern but are the result of an underlying mathematical process
...
Output from models can often be compared to observable values in the market (e.g. market prices, implied volatility and implied correlations)

Differences between market values and modeled values should be

* Explained, or
* Identified as being indicative of possible error (e.g. model error, data error)
