...
* Co-monotonicity (or minium) copula
...
* i.e. when one is high and on is low and no special relationship whne both are low or both are high
...
The fundamentl copulas are specific cases of the general Fréchet-Höffding family of copulas which are of the form
...
* Small number of parametes $(< 3)$ $\Rightarrow$ Application to heterogeneous groups of variables is limited
...
The upper tail dependency (but no lower tail dependency) of the Gumbel makes it suitable for modeling situtations where associations increase for extreme high values (but not for extreme low values)
...
* Else it does not satistfy the fundamental conditions required for a copula
...
* Can be consider when modeling joint and las survivor annuities and exchange rate movements
...
Important to note that the degree of tail dependency (where it exisits) is a function of the parameter(s) of the copula

![](figures/figure-18.07.png)
![](figures/figure-18.08.png)
![](figures/figure-18.09.png)

Difficult to see the differences between the example distribution fucntions above
