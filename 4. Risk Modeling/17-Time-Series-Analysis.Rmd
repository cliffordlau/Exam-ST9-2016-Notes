...
* For all integers $t$: 
    * $\mathrm{Corr}(\epsilon_t,\epsilon_{t+k}) = \begin{cases} 1 & \text{if } k = 0 \\ 0 & \text{if } k \neq 0 \\ \end{cases}$
    * $m(t) = \mathrm{E}(\epsilon_t) = 0$
    
Under a white noise process, every element is uncorrelated with any previous observations and the process oscillates randomly around zero
...
* A set of *iid* r.v. with finite variance
* (Typically) assume $N(0,\sigma^2)$

### Trend and Difference Stationarity

***Trend Stationary***

Trend stationary process is one where the observations oscillate randomly around a steadily changing value which is a function of time only e.g. $X_t = \alpha_0 + \alpha_1 t + \epsilon_t$
...
Integrated process of order $d$ ($I(d)$ process) is one where the process needs to be differenced $d$ times before the result $\Delta^d X_t$ is covariance stationary
...
* $\Delta X_t = \alpha_0 + \alpha_1t + \alpha_2X_{t-1} + \epsilon_t$
* Test statistic $\alpha_2 / s.e.(\alpha_2)$ is compared to the critical values determined by Dickey-Fuller
...
* To avoid negative values: $X_t = \alpha_0 + \alpha_1 X_{t-1} + \sqrt{X_{t-1}}\epsilon_t$
...
If $\mid \alpha_1 \mid < 1$ then the process is mean reverting (\therefore covariance stationary)

* Mean: $\dfrac{\alpha_0}{1-\alpha_1}$
* Variance: $\dfrac{\sigma^2}{1 - \alpha_1^2}$; $\sigma^2$ is the variance of the white noise ($\epsilon$)

If $\mid \alpha_1 \mid > 1$ then the process becomes unstable

If $\mid \alpha_1 \mid = 1$ then the process is a **random walk**

* If addition $\alpha_0 \neq 0$ then the process is a random walk **with drift** (this is difference stationary)
...
$\{\phi_h : h=1,2,...\}$ is defined as the conditional correlation of $X_{t+h}$ with $X_t$ given $X_{t+1},...,X_{t+h+1}$
...
Separate correlograms might be constructed for a variety of degrees of integration (e.g. d = 0, 1, 2) so as to check visually for both serial correlation, degree of integration and the form of model fit
...
Use test statistics to compare between models under consideration (e.g. AIC and BIC in Mod 19)

Can also consider error terms (residuals) arising after having fitted a particular model
...
$\begin{align}
  \mathrm{E}[X_{t+h}] &= \hat{\alpha}_0 (1 + \hat{\alpha}_1 + \dots + \hat{\alpha}^{h-1}_1) + \hat{\alpha}^h_1 X_t + \hat{\alpha}_1^{h-1} \hat{\beta}_1 \hat{\epsilon}_t \\
...
Drift ($\alpha_0$) and/or degree of mean reversion ($\alpha_1$) in the $AR$ model ($X_t = \alpha_0 + \alpha_1 X_{t-1} + \epsilon_t$) might be dependent upon time

Alternatively, the rate of trend ($\alpha_1$) in a trend stationary model ($X_t = \alpha_0 + \alpha_1 t + \epsilon_t$) might be also dependent upon time
...
3. Test statistics:  
$CT = \dfrac{(SSR - (SSR_1 + SSR_2))/k}{(SSR_1 + SSR_2)/(N_1 + N_2 - 2k)}$
...
4. Test statistics has an *F*-distribution with $k$ and $N_1 + N_2 -2k$ d.f.
    * $H_0$ is that there is no structural break (i.e. the parameters of the models fitted to the subseries are not significantly different from that of the model fitted to the complete series)
...
Given the $\{X_t\}$ is weakly stationary then $\mathrm{E}(X^2_t) < \infty$ and:

$\mathrm{Var}\left(X_t \mid F_{t-1}\right) = \mathrm{E}\left(\sigma_t^2 Z_t^2 \mid F_{t-1}\right) = \sigma^2_t \mathrm{Var}(Z_t) = \sigma^2_t = \alpha_0 + \sum \limits_{i=1}^p \alpha_i X_{t-i}^2$

* $F_{t-1}$ is the history of the process up to time $t-1$

As seen above, the conditional variance of $\{X_t\}$ is changing over time

* Property of heteroscedasticity
* Function of the previous squared values of the process
* Large change in the value of the process is often followed by a period of high volatility $\therefore$ Model incorporates the feature of volatility clustering

Although the conditional variance is changing, we don't know in advance when periods of high and low future volatility will occur $\therefore$ Still possible for the $ARCH$ process to be stationary
...
IGARCH occurs when $\sum \limits_{i=1}^p \alpha_i + \sum \limits_{j=1}^q \beta_j = 1$

For an $IGRACH(1,1)$ process $\beta_1 = 1 - \alpha_1$, so:

$\Delta X_t^2 = \alpha_0 - (1- \alpha_1)V_{t-1} + V_t$

Similar to $ARIMA(0,1,1)$ model for $\{X_t^2\}$ although $\{V_t\}$ is not white noise

#### ARMA model with GARCH errors

First fit the model to $ARMA$ and then fit a $GARCH$ model to the resulting residuals (if residuals are not white noise)
...
PDF of $X_0$ is not known but we get around this by considering the conditional likelihood given $X_0 = x_0$

Although $\sigma_0$ is not actually observed, we can choose a starting value for it, which might be the sample s.d. of $X_1,...,X_T$ or just 0
...
Calculated using: $\hat{Z}_t = \dfrac{\hat{\epsilon_t}}{\hat{\sigma}_t}$

* $\hat{\sigma^2_t} = \hat{\alpha}_0 + \sum \limits_{i=1}^{p_2} \hat{\alpha}_i \hat{\epsilon}_{t-i}^2 + \sum \limits_{j=1}^{q_2} \hat{\beta}_j\hat{\sigma}_{t-j}^2$

Need initial values to use the equations

* Can set starting $\hat{\sigma}_t =0$ and $\hat{\sigma}_t = 0$ (or sample s.d. of the observed time series)
...
Data $X_{t-n+1},...,X_t$ from a $GARCH$ model and we want to predict values of $\sigma_{t+h}$ for $h \geq 1$ from a fitted $GARCH(1,1)$ model

Let $F_t$ be the infinite history of the process up to time $t$ and assume that the model is weakly stationary so the $\mathrm{E}(X^2_t) = \mathrm{E}(\sigma^2_t) < \infty$:
...
* Also $\mathrm{E}(X^2_{t+1} \mid F_t) = \sigma^2_{t+1}$

Problem with the above is that we don't have infinite history of the process

* We can proceed by approximating $\sigma^2_t$ using the residual equations in the [GARCH section](#GARCH)
* Approximate forecast of $X_{t+1}^2$ can also be used as an estimate of $\sigma^2_{t+1}$:
    * $\hat{\sigma}^2_{t+1} = \hat{\mathrm{E}}(X_{t+1}^2 \mid F_t) = \alpha_0 + \alpha_1 X^2_t + \beta_1 \hat{\sigma}^2_t$
    
Equation above is a recursive way of estimating volatility one step ahead

Suppose we want to estimate $X_{t+h}$ and $\sigma_{t+h}$ given the information up to $t$

* Both $X_{t+h}$ and $\sigma_{t+h}$ are r.v.
* $\begin{align}
...
Problem with time series is that insufficient data may be available to generate statistics based on a long timescale (e.g. annual volatility)

* In such case we may need to scale up statistics (e.g. estimate annual volatility from monthly volatility)

If data is available from $N$ groups each of which is ***iid* normal*** with means $\bar{\mathbf{X}}$ and covariances given by the $N \times N$ matrix $\boldsymbol{\Sigma}$, then the means and covariances working over a timescale $T$ times as long are $T \bar{\mathbf{X}}$ and $T\boldsymbol{\sigma}$
...
Similarly if $\bar{\mathbf{X}} = \mathbf{0}$ then aggregate volatility for the longer timescale can be derived as $\sqrt{T}$ multiplied by the corresponding volatility for the shorter timescale

Annual VaR might be derived from the monthly VaR my multiplying by $\sqrt{12}$ (i.e. the square root of time rule)

Such scaling becomes **inaccurate if the data is not *iid* normal**

* e.g. if its skewed of there is serial correlation

Better approach in such situations is to parameterize as stochastic model, using the shorter timescale data and then to derive the longer timescale statistics from a set of simulations