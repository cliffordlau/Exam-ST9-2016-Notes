...
* Potential undersirable use of a "catch all" weighting for (heterogeneous) undefined asset class
...
Determines the degree to whih an org's financial position (e.g. solvency) is affected by the impact that change in a single underlying risk factor (e.g. short term interest rates) has on the value of assets and liabilities
...
* Not assessinga wider range of risks (since it focus on a single risk factor)
...
* Requires calculation of the potential prospective average XS return and tracking erros
...
* Sidenote: $\{x : P(x)\}$ means the set of all x for which $\P(x)$ is true; $\inf$ basically means the largest element of the set
...
High confidence level is typically used for capital adquacy purposes
...
Given value $X_t$ of porfolio $X$ at time $t$
...
* Calculate with linera interpolation for the 2^nd^ case
* $t_-$ and $t_+$ are the intergers immediately either side fo $T \times \alpha$
...
* Reliance on past data having catpured all possible future scenarios
* Implies past data is indicative of future experience
* Does no faciliate stress or scenario testing
...
* Gives a different answer each time (only slighly different ideally)
...
* $VaR$ in terms of returns rather than lossess (e.g. based on log asset values)
...
Expected loss given that a loss over the specified $VaR$ has occured
...
VaR draws a line between everyay and exceptional losses
...
* What might happen in the intervening period (e.g. liquiditiy problems)

Key factors in choicing time horizon
...
* Risk in highly liquid investments can normally be reduced very quikcly
...
* Allowance for credi (e.g. default) risk should be made when settig the discount rate and default rates are linked to other risks which vary over time (e.g. inflation risk)
...
* High discount rate should not be seen as a substitute for a detailed risk analysis as this could lead to the rejection of profitable low risk projects in favor of more profitable projects that carry unaceptable levels of risk
...
Project Market Line (PML) is obtained by reexpressing $r_X$ as the required return on project $X$ and re-expressing $U$ as the existing project portfolio

Thte PML shows that a higher returns is required of a project which, in the context of other project opportunities, exposes and org. to greater uncertainty (higher value of $\dfrac{sigma_X}{sigm_U}) and/or has a lower diversification benefit (higher value of $\rho_{X,U}$)
