...
Will discuss how to implement, methods and outputs of the process as well as the issues involved in identifying emerging risks and the problem of bias
...
* e.g. probability of occurrence within a year and financial or reputational impact
...
* Undertake risk assessments; top down (e.g. interviews) and bottom-up (e.g. workshops)
...
Variety of techniques to identify risk
...
Facilitated by an external consultant and requires all participants to be in the same location at the same time
...
* Uneven participation can lead to an incomplete or biased identification of risks
    * To mitigate this, participants should come from various departments across the org. and have different backgrounds
...
Each member then rank each of the risk independently (to avoid group think) and the results are combined to form an overall ranking
...
* Time consuming and expensive $\Rightarrow$ Restrictions on the number of interviews
...
* Risk response action (retain, remove, reduce, or transfer), its cost and expected residual/secondary risk
* Individual involved in monitoring and managing the risk (risk owner)
* Document control information (When was the last update and by whom)

## Risk Concepts and Initial Risk Assessment Techniques
...
* Length of time for which an organization is exposed to a risk
* or, Time required to recover from the event
...
* For equity holders:  
Returns should be judged relative to the level of risk capital (to adjust for risk)
...
* 0-25%, 25%-50%, etc
* low/mid/high
* Score for both freq and sev and multiply together for a risk rating

***Probability Distribution***

Can choose different distribution depending on the data available

#### Risk Mapping

Technique used to illustrate the effect that each risk might have on an org.

Plots each risk on the risk map (axes are the freq and sev)
...
* Change in nature of an existing or known risk, or
* Change in the underlying effectiveness of risk management approaches of an existing or known risk, or
* Development of a new risk (no explicit allowance in existing framework)
    * Generally with much higher level of uncertainty
    
Emerging risks are important:
...
Emerging risks of the past are known risks today e.g. cost of guaranteed annuity rates, health damage from asbestos
...
* Claims from unexpected sources, e.g.
    * Nanotech
    * Mobile phone use
    * GMO food
    * Cyber
...
There won't be definitive study on the emerging risk $\therefore$ need to assess from different angles and sources
...
* Can be due to lack of supportive risk culture
...
In sub optimal culture, reporting of risk can be subject to bias

* Especially in project appraisal where the project champions tend to minimize the risks in hope of getting approval
...
Bias can also take place in reporting to the Board about the ongoing risks facing the enterprise
...
Traditional economic theory assumes investors always act rationally while evidence suggest that this may not always be true
...
* Caveat: If the project is to proceed, it give the PMO a large contingency allowance in the capital cost $\therefore$ reducing incentives to keep capital costs to a minimum