...
* Extreme returns in one series often coincide with extreme returns in several other series (i.e. level of dependence are higher during periods of high volatility)
...
Goal of the PCA is to determine only the main factors that contribute to deviations from the average returns and ignore the factors with less influence $\Rightarrow$ Reducing the complexity (dimensionality) of the analysis

PCA is particularly useful when projecting returns on `bonds` (with a variety of duration)
...
* Need to **further adjust** the average to reflect any `changes that might be expected in the future` (could be subjective or based on fundamental structural changes in the asset classes)
...
If the benchmark is to be based on the liabilities then the reference point is often a set of cashflows (or notional assets) that represent the actual (uncertain) liabilities
...
Exam Note: Need to be familiar with manipulating interest rates (e.g. forward rates and yield curves)
...
* e.g. short term ($r_{1,t}$) and long-term ($r_{2,t}$) at time $t$
    ...
    $\Deltar_{1,t} = [\alpha_a + \beta_1(r_{2,t-1} - r_{1,t-1})] \Delta t + r_{1,t-1}\epsilon_{1,t}$
    $\Deltar_{2,t} = [(\alpha_2 + \beta_2r_{1,t-1} - \gamma_2r_{2,t-1}] \Delta t + r_{2,t-1}\epsilon_{2,t}$
...
* Changes in long term rates vary in population to the square of the level of long term rates (i.e. $\gamma_2 r^2_{2,t-1}$ for some $\gamma_2$), but are also influenced by the short-term rates through the produce team $\beta_2 r_{1,t-1}r_{2,t-1}$
...
For `bond prices` ($p_t$), first calculate **log returns** $\ln (p_t/p_{t-1})$ and then the deviations from average log-returns are analyzed
...
FX risk can be hedged away using currency forward agreements (See Mod 27), hence there is no additional currency return to be gained (or modeled) if working in a single denomination

No-arbitrage argument:

$\dfrac{e_0}{e_t}(1+r_{Y,t}) = r + r_{X,t}$
...
Contagion risks are usually seen as an extension of market risk, but can also apply to other risk (e.g. credit) where the default of one org can cause its creditors and supplier to experience financial difficulties

Contagion risks can be modeled as the **interaction** between different financial series
...
However, that would presents an arbitrage opportunity which in theory should be eliminated by arbitrageurs $\therefore$ such serial correlation effects are usually ignored when modeling
...
* Where the normal level of dependency $\rho_0$ is adjusted by additional dependency $\rho_1$ and $\rho_2$ in different states
* $D_i$ are the states indicator:
