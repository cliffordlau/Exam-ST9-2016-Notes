...
* Should focus on understanding the broad application of the concepts (pros and cons, and difficulties involved)
...
Mod 28 will discuss the applications of some credit derivative tools
...
:   Risk of loss due to **contractual obligations not being met** (in terms of `quantity`, `quality`, or `timing`) either in part of in full, whether due to inability of, or decision by, the counterparty
...
For `derivatives`, the **value fluctuate** with market movement so even though the exposure at an exact point in time may be known, it should not be treated as the full extent of the exposure to the counterparty
...
But even so market risk and credit risk are **not indepdenent** and the inter relationship needs to be considered in the risk assessment; e.g.
...
Recall example of S&P's process from Mod 7
...
4. **Ratings may fail to respond** to changes in the economic cycle or circumstances of the counterparty (particularly there is often a reluctance to change a credit rating)
    * A decision making bias (anchoring)
...
Forecast the likelihood of a counterparty defaulting at a particular point in time given certain `fundamental information` about the counterparty
    ...
    e.g. **empirical models**: analyse the incidence of default in the past for companies with a certain level of gearing, cashflow, profits etc;  
    **expert models**: use the opinions of experts to assess the likelihood of default
...
Does not model the mechanism leading to default; Model it as a statistical process that typically depends upon economic variables

    e.g. Includes all **credit migration models** that estimate how a counterpatyr's credit rating might behave over time; By combining credit ratings and default probabilities (e.g. from a structural model) enable estimation of the overall likelihood of default in a particular future time period
...
Model risk from **model combinations** can be significant as the results can vary significantly depending on the mixing distribution model selected (esp. for the extreme tail)
...
Based on model proposed by Merton in 1974 and uses `option pricing theory` along with `equity share price volatility` to derive information on the value of the company's total assets and hence the value of its debt

From the s/h p.o.v. the equity of a company is a call option on its total assets

* If the total asset > debt at the time the debt has to be repaid, then the s/h will repay the debt and own the company's total assets
* If assets < debt, the s/h will walk away

Similarly: Value of debt =  value of risk-free bond - value of a put option on the company's total asset

* If assets > debt, bondholders will get the same amount at maturity as the holder of a risk-free bond
* If asset < debt, bond holder will lose the difference between the value of the company's total assets at redemption and the value of the debt
...
So we use Ito's Lemma to get a second equation with the same two unknown and solve simultaneously
...
Another limitation: results can be affected significantly by changes in market sentiment in the absence of any real changes in a counterparty's prospects

### KMV Model

Based on the same concept as Merton where a company will default when $X_t$ falls below $B$ (or $\tilde{B}$ which is based on the term structure of all the company's liabilities, e.g. often taken as the liabilities falling within one year)
...
* $X_0$ is not assumed to be observable and is derived from the value of the company's equity shares

However, can similarly be affected significantly by changes in market sentiment in the absence of any real changes in a counterparty's prospects
...
* Model does not rely on publicly traded share information
...
* Time homogeneity assumption has been criticized using empirical evidence and appears unintuitive (e.g. recently downgrade company is more likely to be downgraded again than company that has been at the rating for a long time)
...
* Ratings are sometimes unavailable (withdrawn) e.g. if data required for rating has not been made available

Simpler form of credit migration model assumes that credit migration follows a `martingale process`

* However (for highest rating), it is incorrect to assume that the expected rating one time period on is the same as in the current period $\Rightarrow$ Multi year periods results is a very rough estimates for default
...
Key challenge is to understand the relationships between the various credit exposures so as to model the appropriate behavior (e.g. jointly-fat tails)
...
Extending the credit migration models and assume each org in the portfolio has asset values that behave independently and log-normally we can derive a model of the number of organization that default in each year
...
* Implied changes in the value of the bonds in the portfolio (incl. the default experience)
...
* Change in rating is a function of change in the value of an org's assets and the volatility of the value of those assets
...
For a portfolio of $N$ bonds and $M = 2^N-1$ distinct shocks
...
* Setting $\alpha$ to the default CDF $F(t) = 1 - e^{-ht}$ we can solve for $h = \dfrac{-\ln(1-\alpha)}{t}$
...
2. **Ultimate recovery**
    ...
    Typically much larger than price after default and is not usually known until after 1 or 2 years after default
